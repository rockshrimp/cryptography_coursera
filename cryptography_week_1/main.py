import codecs
import string

c_1 = ""
c_2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
c_3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
c_4 = ""
c_5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
c_6 = ""
c_7 = ""
c_8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
c_9 = ""
c_10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"

def xor_hex_strings(a, b):
    if len(a) > len(b):
        return "".join([hex(int(x, 16) ^ int(y, 16))[2:] for (x, y) in zip(a[:len(b)], b)])
    else:
        return "".join([hex(int(x, 16) ^ int(y, 16))[2:] for (x, y) in zip(a, b[:len(a)])])


def str_to_hex(word):
    return ''.join(hex(ord(c))[2:] for c in word)


def hex_to_ascii(word):
    try:
        return codecs.decode(word, "hex").decode('utf-8')
    except:
        pass


if __name__ == '__main__':
    ciphers = [c_1, c_2, c_3, c_4, c_5]

    guess_word = "the"
    guess_word_hex = str_to_hex(guess_word)

    c_a = c_2
    c_b = c_3

    c_a_xor_c_b = xor_hex_strings(c_a, c_b)

    # Xor with a space only change the case of the letter
    # print("a^\" \" = ", chr(ord("a") ^ ord(" "))) => A
    # print("A^\" \" = ", chr(ord("A") ^ ord(" "))) => a

    for i in range(0, len(c_a_xor_c_b) - len(guess_word_hex), 2):
        cipher_segment = c_a_xor_c_b[i:i + len(guess_word_hex):]
        cipher_segment_xor_guess_word_hex = xor_hex_strings(cipher_segment, guess_word_hex)

        cipher_xor_guess_word_ascii = hex_to_ascii(cipher_segment_xor_guess_word_hex)

        print(str(i) + "   " + hex_to_ascii(cipher_segment_xor_guess_word_hex))

    #We find that trying to to xor "the" at the 80th position gives "ryp"

    def try_string(guess, string_index):
        guess_word_hex = str_to_hex(guess)
        print(hex_to_ascii(xor_hex_strings(c_a_xor_c_b[string_index:string_index + len(guess_word_hex):], guess_word_hex)))

    ####
    print("\n--------\n")
    ####
    i = 80
    guess_word = "ryp"
    guess_word_hex = str_to_hex(guess_word)
    print(hex_to_ascii(xor_hex_strings(c_a_xor_c_b[i:i + len(guess_word_hex):], guess_word_hex)))
    ####
    print("\n--------\n")
    ####


    i = 78
    guess_word = "cryptographer"
    guess_word_hex = str_to_hex(guess_word)
    print(hex_to_ascii(xor_hex_strings(c_a_xor_c_b[i:i + len(guess_word_hex):], guess_word_hex)))
    #This gives " theorem beco"
    ####
    print("\n--------\n")
    ####

    i = 78
    guess_word = " theorem becomes "
    guess_word_hex = str_to_hex(guess_word)
    print(hex_to_ascii(xor_hex_strings(c_a_xor_c_b[i:i + len(guess_word_hex):], guess_word_hex)))
    #This gives "cryptographers ca"
    ####
    print("\n--------\n")
    ####

    i = 76
    guess_word = " cryptographers can "
    guess_word_hex = str_to_hex(guess_word)
    print(hex_to_ascii(xor_hex_strings(c_a_xor_c_b[i:i + len(guess_word_hex):], guess_word_hex)))
    #This gives "s theorem becomes a "
    ####
    print("\n--------\n")
    ####

    i = 76
    guess_word ="s theorem becomes a "
    guess_word_hex = str_to_hex(guess_word)
    print(hex_to_ascii(xor_hex_strings(c_a_xor_c_b[i:i + len(guess_word_hex):], guess_word_hex)))
    #This gives " cryptographers can "
    ####
    print("\n--------\n")
    ####
